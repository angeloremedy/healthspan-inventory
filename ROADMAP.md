...
- ✅ Manuals: headings keep with the first two paragraphs, no widows or orphans; `pagecheck.py` lint is part of the build; all nine rebuilt with the new sections. Each now opens with a Contents page and closes with "Your pages — the complete directory" — every page the role can open, generated from the app (`directory.js`), checked by `coverage.js`
...
- ✅ Specialist directory: the roster is the HQ accounts with a specialist tag; names printed app-wide come from the account (Specialists, CRM activity, Scorecards, Leaderboard, Set targets, spec page, review); Team field in Team & access groups the review and the presenter list; stray tags (territories, Vacant) are flagged with their amount instead of becoming people
- ✅ Brand inference for Shopify-only SKUs (Mesoestetic, Mark-Vu, Symmed, Zionic, SkinPen packages) from the title — no more "SHOPIFY ONLY"; review rolls lines into brands and matches Targets-tab names loosely; machines recognised from titles (never consumables), lone machine inherits the brand target
