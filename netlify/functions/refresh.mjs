...
export const handler=async(event,context)=>{
  ...
  try{
    ...
    const [ovR,oSKU,oQTY,oDC,inR,poR]=rR;
    ...
    for(let i=0;i<ol;i++){
      ...
      if(br&&ds){const yr=Math.floor((ds-25569)/365.25)+1970;if(yr>=2025)bT.push({branch:br,sku:s,name:s,qty:q,dateSerial:ds,batch:'',expiry:'',order:'',line:''});}
    }
    ...
    for(const p of products){
      const mo=skuMO[p.sku]||{};const av=l6.reduce((a,m)=>a+(mo[m]||0),0)/6;
    ...
    return{
      statusCode:200,
      headers:hdrs,
      body:JSON.stringify({products,batches,monthlyIn:mIn,monthlyOut:mOut,months,valueByLine:vbl,cashExpiring:ce,expiringItems:ei.slice(0,100),branchTransfers:bT.slice(0,300),branchExpirySummary:bes,synced:new Date().toISOString(),elapsed}),
    };
