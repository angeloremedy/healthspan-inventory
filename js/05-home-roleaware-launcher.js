...
function renderHome(){
  ...
  if(ROLE==='admin'){
    sections=[
      ...
      ['Admin',[
        ...
        .concat(isSuper()?[card(go('cutover'),'Cutover switches','Independence & evidence',HI.key),card(go('numbering'),'Document numbering','Prefixes & padding',HI.list),card(go('archive'),'Archive','Deleted records, restorable',HI.list)]:[])]
    ];
    ...
    sections=[
      ['Today',[
        ...
        card(go('pullouts'),'Pull-out requests','Approved ones waiting for release',HI.list,1),
        card(go('po'),'Purchase orders','Ordering & receiving',HI.truck)]],
      ['Warehouse',[
    ...
    sections=[
      ['Today',[
        ...
        card(go('pdc'),'PDC register','Cheques to maturity',HI.card),
        card(go('returns'),'Returns & credit memos','CMs, restock or write-off',HI.bag),
        card(go('pullouts'),'Pull-out requests','Class spend & approvals',HI.list,1)]],
      ['Money',[
        card(go('commissions'),'Commissions','Tiers & payroll CSV',HI.target),
        card(go('valuation'),'Landed cost & valuation','True margins, value at cost',HI.scale),
    ...
    sections=[
      ['The weekly numbers',[
        ...
        card(go('pullouts'),'Pull-out requests','File one \u2014 or approve your class',HI.list,1)]]
    ];
  ...
  '.hmc,.hmc>div{min-width:0}.hmc-t,.hmc-s{overflow-wrap:anywhere}'+
  ...
  '@media(max-width:640px){.hm-hero{padding:20px 18px}.hm-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}'+
  '</style>'+
...
async function renderAR(){
  ...
  $('content').innerHTML=
    ...
    rows.slice(0,150).map(r=>'<tr onclick="showAccountPage(\''+jsq(r.name)+'\')" style="cursor:pointer"><td style="font-weight:600;max-width:240px;overflow:hidden;text-overflow:ellipsis">'+esc(r.name)+'</td>'+
...
function noAcctChanged(){
  ...
  if(sug)html+='<div style="background:var(--am-bg);color:var(--am);border-radius:8px;padding:8px 12px;font-size:12px;margin-top:6px">⚠ Not an existing account — did you mean <a href="#" onclick="$(\'no-acct\').value=\''+jsq(sug)+'\';noAcctChanged();return false" style="color:var(--am);font-weight:700">'+esc(sug)+'</a>? Tap to use it, or continue if it’s truly new.</div>';
...
async function renderSpecPage(){
  ...
  if(CAL_SEL){
    ...
    dayHtml='<div class="panel" style="padding:14px;margin-top:10px"><div class="phd">'+esc(CAL_SEL)+'</div>'+
      ...
      e.plan.map(v=>row('<span class="pill pbl">planned</span>',esc(v.account),esc(v.type||'')+(v.notes?' · '+esc(v.notes):''),'showAccountPage(\''+jsq(v.account)+'\')')).join('')+
      e.visit.map(v=>row('<span class="pill" style="background:var(--am-bg);color:var(--am)">visit</span>',esc(v.account),esc(v.type||'')+' · '+esc(v.outcome||''),'showAccountPage(\''+jsq(v.account)+'\')')).join('')+
  ...
  $('content').innerHTML=
    ...
    (fus.length?fus.slice(0,10).map(v=>'<div class="drow" onclick="showAccountPage(\''+jsq(v.account)+'\')" style="cursor:pointer;align-items:flex-start"><span class="dlbl"><b>'+esc(v.account)+'</b> · '+esc(v.date)+'<br><span style="color:var(--tx3);font-size:11px">'+esc(v.notes||'')+'</span></span></div>').join(''):'<div style="font-size:12px;color:var(--tx3)">None — clear!</div>')+'</div>'+
...
async function renderFulfillQ(){
  ...
  $('content').innerHTML=boPanel+
    ...
    '</div>'+
    (pend.length?
    (canFulfil()?'<div style="display:flex;gap:10px;align-items:center;margin-bottom:10px;flex-wrap:wrap">'+
      '<button onclick="waveRelease()" style="background:var(--pu);color:#fff;border:none;border-radius:8px;padding:9px 16px;font-size:12.5px;font-weight:600;cursor:pointer">⚡ Release wave from ticked orders</button>'+
      '<span class="mu" style="font-size:11.5px">tick two or more, get ONE pick list sorted by bin — one walk of the warehouse instead of one per order</span></div>':'')+
    '<div class="tcard"><div class="tscroll"><table><thead><tr>'+(canFulfil()?'<th class="no-print" style="width:26px"></th>':'')+'<th>Order</th><th>Date</th><th style="text-align:right">Age</th><th>Account</th><th>Specialist</th><th style="text-align:right">Items</th><th style="text-align:right">Total</th><th class="no-print"></th></tr></thead><tbody>'+
    pend.map(o=>'<tr onclick="showOrderPage(\''+o.id+'\')" style="cursor:pointer">'+
      (canFulfil()?'<td class="no-print" onclick="event.stopPropagation()"><input type="checkbox" class="wv-pick" value="'+esc(String(o.id))+'" style="width:17px;height:17px"></td>':'')+
      '<td style="font-weight:700">'+esc(ordLabel(o))+'</td><td class="mu">'+esc(o.date)+'</td>'+
...
function renderSalesField(){
  ...
  $('content').innerHTML=
    ...
    (F.notVisited.length?'<div class="tcard"><div class="tscroll"><table><thead><tr><th>Account not reached '+spLbl()+'</th><th>Usual specialist</th><th style="text-align:right">Last booking</th><th style="text-align:right">Booked ₱ (13mo)</th></tr></thead><tbody>'+
      F.notVisited.slice(0,60).map(x=>'<tr onclick="openAccountDrawer(\''+jsq(x.c)+'\')" style="cursor:pointer"><td style="font-weight:600;max-width:250px;overflow:hidden;text-overflow:ellipsis">'+esc(x.c)+'</td><td>'+esc(x.spec||'—')+'</td><td class="r mu">'+esc(x.last||'—')+'</td><td class="r" style="font-weight:600">'+fmtPeso(x.v)+'</td></tr>').join('')+
