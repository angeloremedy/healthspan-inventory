...
const DESC={
  ...
  orgchart:'Who reports to whom across Healthspan Global — names, titles and the reporting line, as the People team keeps it. Tap a name for the person\'s card and their pages in HQ. No pay, no costs: names and titles only.',
  qbo:'HQ → QuickBooks Online, for HQ and Shopify orders alike: an order becomes an invoice when it is created (list price + Discount row, VAT codes per line, class Sales), returns become credit memos, payments recorded in QuickBooks come back. The shadow reconciliation proves HQ matches the old Shopify connector before the switch. Preview until enabled.',
  ...
  receiving:'Inbound shipments from suppliers, one record per delivery against a PO: carrier, tracking, ETD/ETA, customs, broker; the count at the door per line (batch, expiry, bin, QA hold) that writes the stock ledger and the PO; the supplier’s terms turned into a payment due date; and the landed-cost calculator — invoice × FX plus freight, insurance, duty, VAT (recoverable by default), brokerage, arrastre, storage, trucking, bank charges — allocated to a real ₱ cost per unit and applied to the PO for valuation. Short or over counts raise a claim with the supplier in one tap.',
...
/* ── DOCUMENT NUMBERING ─────────────────────────────────────────────────────
   Every printed number (HS-1042, QT-1007, CM-1003, PL-1009, PO-1021, TR-1004)
   used to be a hard-coded one-liner. They now come from one table the super
   admin controls: prefix, zero-padding, and the number the series appears to
   start at. The underlying row id never changes — this is presentation — so
   renumbering can never collide two documents onto one number. Permanent DR
   numbers already stamped on orders are stored, not derived, and are untouched. */
let DOCFMT=null;
const DOCFMT_DEFAULT={order:{prefix:'HS-',pad:0,offset_no:1000},quote:{prefix:'QT-',pad:4,offset_no:0} /* QT-0007, not QT-1007 */,
  cm:{prefix:'CM-',pad:0,offset_no:1000},pullout:{prefix:'PL-',pad:0,offset_no:1000},
  po:{prefix:'PO-',pad:0,offset_no:1000},transfer:{prefix:'TR-',pad:0,offset_no:1000},
  complaint:{prefix:'C-',pad:0,offset_no:0},
  voucher:{prefix:'V-',pad:0,offset_no:1000},orderpay:{prefix:'RO-',pad:0,offset_no:1000},
  proofpay:{prefix:'PP-',pad:0,offset_no:1000},replenish:{prefix:'RP-',pad:0,offset_no:1000},
  reimburse:{prefix:'RE-',pad:0,offset_no:1000},cashadvance:{prefix:'CA-',pad:0,offset_no:1000},
  loan:{prefix:'LN-',pad:0,offset_no:100},wave:{prefix:'WV-',pad:0,offset_no:100},
  expreport:{prefix:'ER-',pad:0,offset_no:100}};
async function loadDocFormats(force){
  if(DOCFMT&&!force)return DOCFMT;
  DOCFMT=Object.assign({},DOCFMT_DEFAULT);
  try{
    const {data}=await SB.from('doc_formats').select('*');
    (data||[]).forEach(r=>{DOCFMT[r.kind]={prefix:r.prefix||'',pad:r.pad||0,offset_no:(r.offset_no==null?1000:r.offset_no),label:r.label};});
  }catch(e){} // pre-migration or offline: the defaults ARE today's behaviour
  return DOCFMT;
}
function docNo(kind,n){
  const f=(DOCFMT&&DOCFMT[kind])||DOCFMT_DEFAULT[kind]||{prefix:'',pad:0,offset_no:0};
  let num=String((f.offset_no||0)+Number(n||0));
...
/* Safe interpolation of a string into a JS string literal inside an HTML
   attribute: onclick="f('HERE')". The old idiom jsq(x)
...
function applySync(data){
  ...
  else if(currentView==='quarantine') renderQuarantine();
  else if(currentView==='pullouts') renderPullouts();
  else if(currentView==='voucher') renderFinForm('voucher');
  else if(currentView==='orderpay') renderFinForm('orderpay');
  else if(currentView==='proofpay') renderFinForm('proofpay');
  else if(currentView==='replenish') renderFinForm('replenish');
  else if(currentView==='reimburse') renderFinForm('reimburse');
  else if(currentView==='cashadvance') renderFinForm('cashadvance');
  else if(currentView==='codelists') renderCodeLists();
  else if(currentView==='routes') renderRoutes();
  else if(currentView==='archive') renderArchive();
  else if(currentView==='numbering') renderNumbering();
  else if(currentView==='shortdated') renderShortDated();
  else if(currentView==='poscore') renderPoScore();
  else if(currentView==='whkpi') renderWhKpi();
  ...
  else if(currentView==='valuation') renderValuation();
